...
function tiles(exp,el) {
    ...
    switch (exp) {
      ...
      case 'template':
        ...
        html = (
          <>
          <select className="border " name="template" id="template">
          <option value="default">Default</option>
          <option value="all">All</option>
        </select>
        <button onClick={() => addAll()} className="bg-blue-400 rounded p-4 text-white" >
        Use
        </button>
        </>
        )
...
return (
  ...
  async function addAll() {
    ...
    var data = element
      data = {data: JSON.parse(data.data), description: data.description, title: data.title}
      data = data.data
    if(data.types === 'image') {
      ...
      var response = await fetch(data.api, {
      	...
      	layoutnew = layoutnew.concat( [{i: String(i), x:layoutItem.x+i % 13,y:layoutItem.y,w:data.w,h:data.h,name: data.name,data:await response.data,type: data.types}])
        console.log(layoutnew)
      setLayout(layoutnew);

      setLayouts({
        lg: newlayout.concat([{i: String(i), x:layoutItem.x+i % 13,y:layoutItem.y,w:data.w,h:data.h,name: data.name,data:await response.data,type: data.types}]),
        md: newlayout.concat([{i: String(i), x:layoutItem.x+i % 13,y:layoutItem.y,w:data.w,h:data.h,name: data.name,data:await response.data,type: data.types}]), 
        sm: newlayout.concat([{i: String(i), x:layoutItem.x+i % 13,y:layoutItem.y,w:data.w,h:data.h,name: data.name,data:await response.data,type: data.types}]),
        xs: newlayout.concat([{i: String(i), x:layoutItem.x+i % 13,y:layoutItem.y,w:data.w,h:data.h,name: data.name,data:await response.data,type: data.types}]), 
        xxs: newlayout.concat([{i: String(i), x:layoutItem.x+i % 13,y:layoutItem.y,w:data.w,h:data.h,name: data.name,data:await response.data,type: data.types}])
    })
    i++;
